...
// PostgreSQL connection configuration using environment variablesconst pool = new Pool({
  connectionString: process.env.DATABASE_URL || 'postgresql://postgres:postgres@db:5432/postgres'});

app.get('/', async (req, res) => {
  try {
    // Quick query to test database connectivityconst dbRes = await pool.query('SELECT NOW()');
    res.json({
      status: "healthy",
